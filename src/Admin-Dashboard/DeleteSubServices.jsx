...
import { deleteEntity, getEntitysList } from '../service/subServService.js';
import { PencilSquareIcon, TrashIcon } from '@heroicons/react/24/solid';
import { useNavigate } from 'react-router-dom';
import { toast, ToastContainer } from "react-toastify";

const DeleteSubServices = () => {
    ...
    useEffect(()=>{
        const getRecordsList = async() =>{
            ...
            {      
                toast.error( "Unable to Load Sub Services: " + error.message )
            }
            finally
            {
                setLoading(false);
    ...
    const editCall = (subserviceid)=>
    {
        navigate( `/admindashboard/edit-sub-service/${subserviceid}` );
    }
    const deleteCall = async (subserviceid) => {
        const confirmbox = confirm("Are you sure you want to delete ?");

        if (confirmbox) {
            try {
                const data = await deleteEntity(subserviceid);
                if (data?.deletedSuccess) {
                    toast.success("Sub Service deleted successfully!");
                    
                    // 1. Calculate the new local array state first
                    const updatedList = allRecordsArr.filter(item => item.subserviceid !== subserviceid);
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-xs">
                    ...
                    <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-700">
                        ...
                        <th onClick={() => handleSort('name')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                            Sub Service Name <SortArrow field="name" />
                        </th>
                        ...
                        <th onClick={() => handleSort('description')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                            Description <SortArrow field="description" />
                        </th>
                    ...
                    <tbody className="divide-y divide-gray-400 divide-x divide-gray-200">
                        ...
                        >
                            ...
                            <td className="py-4 flex flex-row justify-center">
                                <PencilSquareIcon onClick={()=>editCall(record.subserviceid)} className='cursor-pointer size-6 text-blue-500' />
                                <TrashIcon onClick={()=>deleteCall(record.subserviceid)} className='cursor-pointer size-6 text-red-500' />
                            </td>
                        </tr>
                        ))}
                    </tbody>
