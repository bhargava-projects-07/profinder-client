...
import { deleteEntity, getEntitysList } from '../service/ServProviderService.js';
import { PencilSquareIcon, TrashIcon } from '@heroicons/react/24/solid';
import { useNavigate } from 'react-router-dom';
import { toast, ToastContainer } from "react-toastify";

const DeleteServProviders = () => {
    ...
    const editCall = (serviceproviderid)=>
    {
        navigate( `/admindashboard/edit-service-provider/${serviceproviderid}` );
    }
    const deleteCall = async (serviceproviderid) => {
        const confirmbox = confirm("Are you sure you want to delete ?");

        if (confirmbox) {
            try {
                const data = await deleteEntity(serviceproviderid);
                if (data?.deletedSuccess) {
                    toast.success("Sub Service deleted successfully!");
                    
                    // 1. Calculate the new local array state first
                    const updatedList = allRecordsArr.filter(item => item.serviceproviderid !== serviceproviderid);
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-xs">
                    ...
                    <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-700">
                        ...
                        <th onClick={() => handleSort('name')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                            Service Provider Name <SortArrow field="name" />
                        </th>
                        <th onClick={() => handleSort('address')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                            Address <SortArrow field="address" />
                        </th>
                    ...
                    <tbody className="divide-y divide-gray-400 divide-x divide-gray-200">
                        ...
                        >
                            ...
                            <td className="py-4 flex flex-row justify-center">
                                <PencilSquareIcon onClick={()=>editCall(record.serviceproviderid)} className='cursor-pointer size-6 text-blue-500' />
                                <TrashIcon onClick={()=>deleteCall(record.serviceproviderid)} className='cursor-pointer size-6 text-red-500' />
                            </td>
                        </tr>
                        ))}
                    </tbody>
...
export default DeleteServProviders