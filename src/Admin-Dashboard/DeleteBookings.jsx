...
import { deleteEntity, getEntitysList } from '../service/ServBookingService.js';
...
const DeleteBookings = () => {
        ...
        const getRecordsList = async() =>{
            try
            {
                ...
                const dataRecords = data.map((item,index) => ({
                    id: index+1,
                    servicebookingid: item._id,
                    servicename: item.serviceid.name,
                    subservicename: item.subserviceid.name,
                    serviceprovname: item.serviceproviderid.businessname,
                    name: item.name,
            ...
            {      
                toast.error( "Unable to Load Bookings: " + error.message )
            }
            finally
            {
                setLoading(false);
    ...
    const editCall = (servicebookingid)=>
    {
        navigate( `/admindashboard/edit-service-booking/${servicebookingid}` );
    }
    const deleteCall = async (servicebookingid) => {
        const confirmbox = confirm("Are you sure you want to delete ?");

        if (confirmbox) {
            try {
                const data = await deleteEntity(servicebookingid);
                if (data?.deletedSuccess) {
                    toast.success("Service Booking deleted successfully!");
                    
                    // 1. Calculate the new local array state first
                    const updatedList = allRecordsArr.filter(item => item.servicebookingid !== servicebookingid);
                    ...
                    const activeFilteredList = updatedList.filter(item =>
                        ...
                        item.subservicename.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        item.serviceprovname.toLowerCase().includes(searchTerm.toLowerCase()) ||
                    ...
                    toast.info("Could not delete service booking.");
                }
            } catch (error) {
                toast.error("Could not delete service booking : " + error.message);
            }
        }
    }
    ...
    if( loading ) return <p className='mt-5 text-center'>Loading Sub Services List...</p>
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-xs">
                    <table className="min-w-full divide-y divide-gray-200 border-collapse bg-white text-left text-sm text-gray-500">
                        <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-700">
                            {/* --- ADDED: column borders (divide-x) --- */}
                            <tr className="divide-x divide-gray-200">
                            {/* --- NEW: Interactive Sort Headers --- */}


                            <th onClick={() => handleSort('servicename')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                                Service Name <SortArrow field="servicename" />
                            </th>
                            <th onClick={() => handleSort('subservicename')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                                Sub Service <SortArrow field="subservicename" />
                            </th>
                            <th onClick={() => handleSort('serviceprovname')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                                Service Provider <SortArrow field="serviceprovname" />
                            </th>


                            <th onClick={() => handleSort('name')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                                Customer Name <SortArrow field="name" />
                            </th>
                            <th onClick={() => handleSort('message')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                                Message <SortArrow field="message" />
                            </th>
                        ...
                        <tbody className="divide-y divide-gray-400 divide-x divide-gray-200">
                            ...
                            >
                                <td className="px-6 py-4 font-medium text-gray-900">{record.servicename}</td>
                                <td className="px-6 py-4 font-medium text-gray-900">{record.subservicename}</td>
                                <td className="px-6 py-4 font-medium text-gray-900">{record.serviceprovname}</td>
                                ...
                                <td className="py-4 flex flex-row justify-center">
                                    <TrashIcon onClick={()=>deleteCall(record.servicebookingid)} className='cursor-pointer size-6 text-red-500' />
                                </td>
                            </tr>
                            ))}
                        </tbody>
