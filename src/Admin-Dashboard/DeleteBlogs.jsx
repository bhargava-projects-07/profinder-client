...
import { deleteEntity, getEntitysList } from '../service/blogService.js';
...
const DeleteBlogs = () => {
    ...
    const editCall = (blogid)=>
    {
        navigate( `/admindashboard/edit-blog/${blogid}` );
    }
    const deleteCall = async (blogid) => {
        const confirmbox = confirm("Are you sure you want to delete ?");

        if (confirmbox) {
            try {
                const data = await deleteEntity(blogid);
                if (data?.deletedSuccess) {
                    toast.success("Blog deleted successfully!");
                    
                    // 1. Calculate the new local array state first
                    const updatedList = allRecordsArr.filter(item => item.blogid !== blogid);
    ...
    if( loading ) return <p className='mt-5 text-center'>Loading Blogs List...</p>
                    ...
                    <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-700">
                        ...
                        <tr className="divide-x divide-gray-200">
                            <th onClick={() => handleSort('title')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                                Blog Title <SortArrow field="title" />
                            </th>
                            <th onClick={() => handleSort('content')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                                Blog Content <SortArrow field="content" />
                            </th>
                    ...
                    <tbody className="divide-y divide-gray-400 divide-x divide-gray-200">
                        ...
                        >
                            ...
                            <td className="py-4 flex flex-row justify-center">
                                <PencilSquareIcon onClick={()=>editCall(record.blogid)} className='cursor-pointer size-6 text-blue-500' />
                                <TrashIcon onClick={()=>deleteCall(record.blogid)} className='cursor-pointer size-6 text-red-500' />
                            </td>
                        </tr>
                        ))}
                    </tbody>
