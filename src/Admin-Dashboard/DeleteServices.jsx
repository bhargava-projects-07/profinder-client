...
import { deleteEntity, getEntitysList } from '../service/servService';
...
const DeleteServices = () => {
    ...
    const editCall = (serviceid)=>
    {
        navigate( `/admindashboard/edit-service/${serviceid}` );
    }
    const deleteCall = async (serviceid) => {
        const confirmbox = confirm("Are you sure you want to delete ?");

        if (confirmbox) {
            try {
                const data = await deleteEntity(serviceid);
                if (data?.deletedSuccess) {
                    toast.success("Service deleted successfully!");
                    
                    // 1. Calculate the new local array state first
                    const updatedList = allRecordsArr.filter(item => item.serviceid !== serviceid);
    ...
    if( loading ) return <p className='mt-5 text-center'>Loading Services List...</p>
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-xs">
                    ...
                    <thead className="bg-gray-50 text-xs font-semibold uppercase text-gray-700">
                        ...
                        <th onClick={() => handleSort('name')} className="cursor-pointer px-6 py-3 select-none hover:bg-gray-100">
                            Service Name <SortArrow field="name" />
                        </th>
                    ...
                    <tbody className="divide-y divide-gray-400 divide-x divide-gray-200">
                        ...
                        >
                            ...
                            <td className="py-4 flex flex-row justify-center">
                                <PencilSquareIcon onClick={()=>editCall(record.serviceid)} className='cursor-pointer size-6 text-blue-500' />
                                <TrashIcon onClick={()=>deleteCall(record.serviceid)} className='cursor-pointer size-6 text-red-500' />
                            </td>
                        </tr>
                        ))}
                    </tbody>
