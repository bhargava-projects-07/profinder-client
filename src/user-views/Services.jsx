...
const Services = ({subDisp, home = false }) => {
    ...
    <>
        <main className={`${subDisp || home ? 'mt-8': '' }`}>

            <section className={ home ? "flex flex-row justify-center" : "" }>
                <div className={ subDisp ? 'subDispHeading' : home ? 'homeDispHeading' : 'breadCrumbUserView' }>
                                ...
                                services.map((service,index)=>(
                                    <Grid key={index} size={ ! subDisp ?  {xs: 12, sm: 6, md: 4, lg: 3} : { xs: 12 } }>
                                        <Box component={RouterLink} to={`/service/${service._id}`} sx={{
                                                display: 'block',
                                                bgcolor: (! subDisp && ! home) ? '#E7FDFE' : '',
                                                p: subDisp ? 0 : home ? 1 : 3, 
                                                borderRadius: subDisp ? '2px' : home ? '4px' : '8px',
                                                boxShadow: ! subDisp ? '0px 2px 8px rgba(0, 0, 0, 0.4), 0px 1px 3px rgba(0, 0, 0, 0.5)' : '',
                                                border: (! subDisp && ! home) ? '1px solid #63F0F8' : '',
                                                textAlign: 'center',
                                                textDecoration: 'none',color: 'inherit',cursor: 'pointer'
                                        }}>
                                            <Typography variant={ subDisp ? 'subtitle1': home ? 'h6' : 'h5' } sx={ serviceStyling }>
                                                {service.name}
                                            </Typography>
                                        </Box>
                                    </Grid>
